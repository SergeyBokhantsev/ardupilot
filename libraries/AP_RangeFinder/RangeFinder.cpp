...
void RangeFinder::detect_instance(uint8_t instance, uint8_t& serial_instance)
{
    ...
    case RangeFinder_TYPE_BenewakeTFmini:
        ...
        break;
    case RangeFinder_TYPE_BenewakeTFmini_I2C:
        if (state[instance].address) {
            //if (!_add_backend(AP_RangeFinder_TfMiniI2C::detect(state[instance], hal.i2c_mgr->get_device(1, 0x10)))) 
            {
                _add_backend(AP_RangeFinder_TfMiniI2C::detect(state[instance], hal.i2c_mgr->get_device(1, state[instance].address, state[instance].pin * 1000)));
            }
        }
        break;
    default:
